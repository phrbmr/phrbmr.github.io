...
# O objetivo deste laboratórioé trabalhar com 7 fontes de dado direfentes,
#realizando a limpeza, organização e combinação dos dados, para ao final 
#contrir uma web app de dashboard analítico para compreender a relação do
#faturamento da Netflix com o PIB e indicadores de desigualdade social.

# Os datasets tem a seguinte origem:
...
# a) Carregando os pacotes que serão utliizados ####
library(dplyr)
library(tidyr)
library(readxl)
library(readr)
...
# b) Limpeza e Preparação do Primeiro Dataset Combinado ####

# Contruindo colunas com a diferença de dados para o gráfico de barras
## Plano standard - plano básico
...
## Combinando os dados gerados com os dados do PIB
...
## Limpando o dataframe de desigualdade salarial
dados_salario <- dados_salario[, c(1:3)]
dados_salario_ano <- dados_salario %>% group_by(country) %>% 
...
dados_salario <- merge(dados_salario, dados_salario_ano, 
                       by.x = c("country", "year"), 
                       by.y = c("country", "max"))
dados_netflix_pib_salario2020 <- merge(dados_netflix_pib2020, dados_salario, 
                                       by.x=c("Country"), 
                                       by.y=c("country"))
View(dados_salario)
View(dados_netflix_pib_salario2020)

## Limpando o dataset de faturamento e subscrição, e combinando com o anterior
dados_sub <- dados_sub[,c(1, 23,24)]
complete <- merge(dados_netflix_pib_salario2020, dados_sub, by=c("Country"))
View(complete)

## Fazendoo merge com countrycode, para poder ser usado no choropleth map
countrycode <- countrycode[,c(1, 3)]
complete <- merge(complete, countrycode, by.x=c("Country"), by.y=c("English.short.name.lower.case"))
View(complete)

# Salvando o dataframe produzido até aqui
write.csv(complete, "datasets_limpos/dataset1.csv", row.names = FALSE)


# c) Limpeza e Preparação do Segundo Dataset Combinado ####

# Limpeza e filtro do dataframet do IMDB
...
topgenero <- distinct(topgenero, show_title, week, country_name, category, 
                      titleType,cumulative_weeks_in_top_10, .keep_all= TRUE)
View(topgenero)
...
# Pivot do dataframe
topgeneropaises <- separate(topgeneropaises, 
                            c("genres") , c("genero1", "genero2", "genero3"), sep = ",")
topgeneropaises <- pivot_longer(topgeneropaises, 
                                c("genero1", "genero2", "genero3"), 
...
# d) Limpeza e Preparação do Terceiro Dataset Combinado ####
...
# e) Limpeza e Preparação do Quarto Dataset Combinado ####
...
# f) Limpeza e Preparação do Quinto Dataset Combinado ####
...
# g) Limpeza e Preparação do Sexto Dataset Combinado ####
